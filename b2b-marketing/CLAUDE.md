...
### 成果物レポートの格納先はNotion（リポジトリには置かない）

Twenty CRMはレコード（構造化データ）の保存基盤であり、**レポートという「読み物」の保存先ではない**。
一方でGitHub（本リポジトリ）が持つのは**ルールだけ**であり、成果物は持たない。

| 置き場所 | 役割 | 備考 |
|---------|------|------|
| GitHub（本リポジトリ） | **ルールのみ** | 組織規則・エージェント定義・スキル。**成果物はコミットしない** |
| Notion「⑤ toBマーケティング 成果物レポート」 | **成果物レポート** | `マーケティング研究`ページ配下。子ページとして1レポート1ページ |
| Twenty CRM | **構造化データ** | Company／Person／Opportunity／AbmHypothesis等のレコード |

**禁止事項：** アカウント分析レポート・ステークホルダーマップ・提案書・調査結果などの成果物を
リポジトリ内にファイルとして作成・コミットしないこと。作業用の一時ファイルが必要な場合は
スクラッチパッドを使い、完成物はNotionへ直接作成する。
（`b2b-marketing/accounts/`ディレクトリは廃止済み。過去データはGit履歴を参照）

**Notionへレポートを作成するときの注意：**
