...
|-----|-----|----------------|
| 親ページ「マーケティング研究」 | https://app.notion.com/p/3a0555500af181ff9f47f6073bd00489 | — |
| **①〜④ toCマーケティング 成果物レポート**（成果物レポートの格納先） | https://app.notion.com/p/3ba555500af1817a9015de3faadacd3b | — |
...
## 保存基盤の使い分け：成果物レポートと構造化レポート

toBグループ（⑤ b2b-marketing）の運用ルールに合わせ、toCグループも**成果物レポート**と**構造化レポート**の
2種類に分けてNotionへ格納する。toBと異なり、toCは保存先がすべてNotion内である点が特徴（構造化レポートも
Twenty CRMのような別システムではなくNotionのデータベースに格納する）。

| 種別 | 内容 | 保存先 | 担当エージェント |
|-----|------|-------|--------------|
| **成果物レポート** | 調査レポート・N1分析・施策設計書等の「読み物」全文 | Notion（「①〜④ toCマーケティング 成果物レポート」ページ配下に子ページとして作成） | 各分析エージェント（market-scout／insight-analyst／content-strategist／performance-analyzer等） |
| **構造化レポート** | Market Insights／Hypothesis Log／Campaign Results／Learningsの各レコード（プロパティに整理された検索・分析用データ） | Notion（既存の4データベース） | knowledge-curator |

**重要な原則：成果物レポートと構造化レポートは対で作成する。**

1. 各分析エージェントの成果物（マークダウンの全文レポート）を受け取ったら、まず該当する構造化データベースに
   要点をレコードとして登録する（従来通りの2. ナレッジ保存の手順）
2. 続けて、レポート全文を「①〜④ toCマーケティング 成果物レポート」ページの子ページとして作成する
3. レポートページの末尾に、必ず「## 構造化データへの登録内容」セクションを設け、
   保存したデータベース名・レコードへのリンクを記載し、相互参照できるようにする
...
各エージェントからアウトプットを受け取ったら、**構造化レポート→成果物レポートの順**で2段階で保存する。

**STEP 1：構造化データベースへの登録**（要点をレコード化）
...
**STEP 2：成果物レポートページの作成**（全文を子ページとして保存）

エージェントから受け取ったマークダウン全文を、「①〜④ toCマーケティング 成果物レポート」ページ
（`3ba555500af1817a9015de3faadacd3b`）配下に子ページとして作成する。

```
notion-create-pages:
  parent: { page_id: "3ba555500af1817a9015de3faadacd3b" }
  pages: [{
    properties: { "title": "[アカウント/CEP名] インサイト調査レポート" },
    content: "（受け取ったマークダウン全文）\n\n---\n\n## 構造化データへの登録内容\n- 保存先データベース：Market Insights\n- レコードURL：（STEP1で作成したページのURL）\n- 取得日：2026-07-13"
  }]
```

**この2段階を必ずセットで実施する。** 構造化データのみ・成果物レポートのみの片方だけで終わらせない。
...
- [ ] 構造化データベースへの登録と成果物レポートページの作成を両方実施したか（片方のみで終わっていないか）
- [ ] 成果物レポートページの末尾に「構造化データへの登録内容」（保存先DB名・レコードURL）を記載したか
