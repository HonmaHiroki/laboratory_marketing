...
| `slide-parts-library-400.pptx` | 汎用スライドレイアウト集（401枚）。表紙・目次・章扉・比較表・グラフ・組織図・スケジュール・SNS別レイアウト等、幅広いレイアウトパターンを収録 | proposal-deck-template.pptxの19スライドでは表現しきれないレイアウト（例：3案以上の比較表、業界特化の図解、章区切りページのバリエーション）が必要なとき |
...
表現しきれないセクションがある場合にのみ`slide-parts-library-400.pptx`から個別レイアウトを補って組み込む。
401枚と大容量のため、全体を一度にサムネイル化せず、必要になった時点で`thumbnail.py`を使い該当箇所のみ確認する
（枚数が多い場合は生成に時間がかかるため、目的のセクションが絞り込めている場合は該当スライド番号付近のみ
`python-pptx`等で個別に開いて確認する方法も検討する）。
...
STEP B：必要なスライドを土台に、add_slide.pyで複製する（顧客の情報量に応じて増減）
  ...
  - **19スライドで表現しきれない構成が必要な場合**：slide-parts-library-400.pptxから該当レイアウトを探し、
