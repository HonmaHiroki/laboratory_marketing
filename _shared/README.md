...
## API_SETUP.md
market-scoutが使用する全API・データソースの事前準備ガイド（登録先URL・必要なもの・取得手順・認証情報・費用・注意点）。
新しい環境でデータ収集を始める前に、まずこのファイルで各APIキー・認証情報を取得する。
...
**★このリポジトリ（GitHub）が管理するのは組織のルール（`CLAUDE.md`・`agents/`・`skills/`等）のみであり、個別案件の調査レポート・分析結果・施策提案書などの成果物は絶対にコミットしない。** 成果物は必ずNotion（成果物レポートページ／4データベース）へ格納する（詳細はリポジトリルート`README.md`参照）。
